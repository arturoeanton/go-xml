...
func Set(data map[string]any, path string, value any) error {
	...
	for i, part := range parts {
		...
		if !ok {
			return fmt.Errorf("cannot navigate '%s': current node is not a map", part)
		}

		// 2. Handle Array Access: "tags[1]"
		if idx >= 0 {
			// We must find the array INSIDE the current map first
			arrVal, exists := m[key]
			if !exists {
		...
		// If the next node does not exist, create it as a map
		if _, exists := m[key]; !exists {
			m[key] = make(map[string]any)
		}
		current = m[key] // Advance
	}
	return nil
}
...
func Delete(data map[string]any, path string) error {
	...
	// CASE A: Delete an element from an Array (e.g., "telefonos[1]")
	if idx >= 0 {
	...
	// CASE B: Delete a Map key (e.g., "email")
